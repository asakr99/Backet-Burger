                ...
                <div class="bg-white dark:bg-zinc-800/50 rounded-[2rem] overflow-hidden shadow-lg border border-gray-100 dark:border-zinc-700/50 group hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 flex flex-col h-full">
                    ...
                    <div class="p-6 flex flex-col flex-grow">
                        ...
                        <div class="mt-auto flex justify-between items-end border-t dark:border-zinc-700 pt-4">
                            ${priceDisplayHTML}
                            <button onclick="openAddonModal(${index})" class="w-12 h-12 flex items-center justify-center bg-[#423064] text-white rounded-full font-bold shadow-lg hover:bg-purple-700 hover:scale-110 transition active:scale-95">
...
// فتح النافذة
function openAddonModal(mealIndex) {
    const offers = getJSON('siteOffers');
    const addonGroups = getJSON('siteAddonGroups') || [];
    currentMeal = offers[mealIndex];
    currentQty = 1;
    
    // حساب السعر الأساسي (مع الخصم إن وجد)
    basePrice = parseFloat(currentMeal.price);
    if(currentMeal.discount) {
        basePrice = basePrice - (basePrice * (parseFloat(currentMeal.discount)/100));
    }

    // تعبئة البيانات
    document.getElementById('modal-img').src = currentMeal.img;
    document.getElementById('modal-title').innerText = currentLang === 'ar' ? currentMeal.nameAr : currentMeal.nameEn;
    document.getElementById('modal-desc').innerText = currentLang === 'ar' ? currentMeal.descAr : currentMeal.descEn;
    
    // بناء الخيارات
    const content = document.getElementById('modal-content');
    content.innerHTML = '';

    if(currentMeal.linkedAddons && currentMeal.linkedAddons.length > 0) {
        currentMeal.linkedAddons.forEach(groupId => {
            const group = addonGroups.find(g => g.id === groupId);
            if(group) {
                const groupTitle = currentLang === 'ar' ? group.titleAr : group.titleEn;
                let html = `
                    <div class="addon-group">
                        <h4 class="font-bold text-lg mb-3 dark:text-white flex justify-between">
                            ${groupTitle}
                            ${group.type === 'one' ? '<span class="text-xs bg-red-100 text-red-600 px-2 py-1 rounded">اجباري</span>' : ''}
                        </h4>
                        <div class="space-y-3">`;
                
                group.options.forEach((opt, idx) => {
                    const optName = currentLang === 'ar' ? opt.nameAr : opt.nameEn;
                    const priceTxt = opt.price > 0 ? `+${opt.price}` : '';
                    
                    if(group.type === 'one') {
                        html += `
                            <label class="flex items-center justify-between p-3 rounded-xl border dark:border-zinc-700 cursor-pointer hover:bg-gray-50 dark:hover:bg-zinc-800/50">
                                <div class="flex items-center gap-3">
                                    <input type="radio" name="g-${group.id}" value="${opt.price}" data-name="${optName}" class="w-5 h-5 accent-purple-600" onchange="calcTotal()" ${idx===0 ? 'checked':''}>
                                    <span class="dark:text-gray-300 font-medium">${optName}</span>
                                </div>
                                <span class="text-sm font-bold text-gray-500">${priceTxt}</span>
                            </label>`;
                    } else if(group.type === 'many') {
                        html += `
                            <label class="flex items-center justify-between p-3 rounded-xl border dark:border-zinc-700 cursor-pointer hover:bg-gray-50 dark:hover:bg-zinc-800/50">
                                <div class="flex items-center gap-3">
                                    <input type="checkbox" value="${opt.price}" data-name="${optName}" class="w-5 h-5 accent-purple-600" onchange="calcTotal()">
                                    <span class="dark:text-gray-300 font-medium">${optName}</span>
                                </div>
                                <span class="text-sm font-bold text-gray-500">${priceTxt}</span>
                            </label>`;
                        ...
                        html += `
                            <div class="flex items-center justify-between p-3 rounded-xl border dark:border-zinc-700 hover:bg-gray-50 dark:hover:bg-zinc-800/50">
                                <div class="dark:text-gray-300 font-medium">${optName} <span class="text-xs text-gray-500">(${priceTxt})</span></div>
                                <div class="flex items-center gap-2">
                                    <button onclick="updateAddonQty(this, -1, ${opt.price})" class="w-8 h-8 bg-gray-200 dark:bg-zinc-600 rounded font-bold">-</button>
                                    <input type="number" readonly value="0" class="addon-qty-input w-8 text-center bg-transparent font-bold" data-price="${opt.price}" data-name="${optName}">
                                    <button onclick="updateAddonQty(this, 1, ${opt.price})" class="w-8 h-8 bg-gray-200 dark:bg-zinc-600 rounded font-bold">+</button>
                                </div>
                            </div>`;
                    }
        ...
        content.innerHTML = `<div class="text-center text-gray-400 py-10">لا توجد إضافات لهذه الوجبة.<br>يمكنك إضافتها للسلة مباشرة.</div>`;
    }
...
// دالة جديدة للتحكم بكميات الإضافات (مثل الماء)
function updateAddonQty(btn, change, price) {
...
function calcTotal() {
    addonsTotal = 0;
    // جمع أسعار الراديو (one)
    document.querySelectorAll('input[type=radio]:checked').forEach(el => addonsTotal += parseFloat(el.value));
    // جمع أسعار التشيك بوكس (many)
    document.querySelectorAll('input[type=checkbox]:checked').forEach(el => addonsTotal += parseFloat(el.value));
    // جمع أسعار الكميات (qty)
    document.querySelectorAll('.addon-qty-input').forEach(el => {
        addonsTotal += (parseInt(el.value) * parseFloat(el.dataset.price));
    });
    
    const finalPrice = (basePrice + addonsTotal) * currentQty;
    document.getElementById('modal-total').innerText = finalPrice.toFixed(2);
}

function confirmAddToCart() {
    // جمع أسماء الإضافات المختارة
    let selectedOptions = [];
    
    // الراديو والتشيك بوكس
    document.querySelectorAll('#modal-content input:not(.addon-qty-input):checked').forEach(el => {
        selectedOptions.push(el.getAttribute('data-name'));
    });
    ...
    document.querySelectorAll('.addon-qty-input').forEach(el => {
        ...
        if(qty > 0) {
            selectedOptions.push(`${el.getAttribute('data-name')} (x${qty})`);
        }
    });

    const finalItem = {
        name: currentMeal.nameAr, 
        price: (basePrice + addonsTotal), // نحفظ سعر الوحدة (السعر الأساسي + الإضافات)
        qty: currentQty,
        notes: selectedOptions.join('، ')
    };
    ...
    updateCartCount();
    closeAddonModal();
    showToast(`تمت إضافة <b>${finalItem.name}</b> للسلة`);
}